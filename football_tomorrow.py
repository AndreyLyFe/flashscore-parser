...
def func(a: int, b: int, part_text: str, count: int, letter: str) -> int:
...
print()
input_shans_x = float(input(
    'Введите максимальную вероятность аутсайдера(Максимально-возможная вероятность на то, что выиграет аутсайдер: '))
input_izmen_x = float(
    input('Введите максимальное изменение аутсайдера(Максимальное изменение коэффициентов на аутсайдера): '))
...
try:
    ...
    url = 'https://d.flashscore.com/x/feed/f_1_1_3_ua_1'
    ...
    for _ in range(count_AA):
        ...
        if not text[index_find:index_find + 7] in all_url:
            if text[index_find + 8:index_find + 11] == '¬AD':
                ...
                q = text.find('¬ADE÷') + 5
                w = text.find('¬AB÷')
                all_url[text[:8]] = text[a:b], text[e:d], text[q:w]
        else:
            continue
    ...
    for id in math_hockey_all:
        ...
        try:
            ...
            Excel.wb.save(f"football_tomorrow_{input_shans}_{input_izmen}.xlsx")
